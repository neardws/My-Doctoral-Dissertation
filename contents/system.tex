...
\centering
  ...
  \bicaption{DSRC的应用层传输时延的概率密度函数（PDF）}{Probability density function (PDF) of application-layer transmission delay of DSRC}
  \label{fig 6-3}
\end{figure}
...
\renewcommand{\algorithmcfname}{算法}
	\caption{基于视图修正的碰撞预警（View Calibration Based Collision Warning，简称 VCCW）}
...
\begin{figure}[h]
     \centering
     \subfloat[][]{\includegraphics[width=0.5\columnwidth]{Fig6-4a-map.pdf}}
     \subfloat[][]{\includegraphics[width=0.5\columnwidth]{Fig6-4b-map.pdf}}
     \bicaption[交叉路口车辆轨迹]{交叉路口车辆轨迹。其中交叉路口均无交通信号灯（a）下午7点车辆轨迹且边缘节点位于$(10422.0, 12465.3)$（b）下午6点车辆轨迹且边缘节点位于$(6097.1, 14870.0)$}[Vehicular trajectory in two road intersections without traffic lights in 7 PM and 6 PM, respectively]{Vehicular trajectory in two road intersections without traffic lights in 7 PM and 6 PM, respectively. (a) Edge node located in $(10422.0, 12465.3)$ (b) Edge node located in $(6097.1, 14870.0)$}
     \label{fig 6-4}
\end{figure}
...
\begin{figure}[h]
     ...
     \bicaption[不同时距阈值下的性能对比]{不同时距阈值下的性能对比，其中时距阈值从1s 增加至 5s。（a）查准率（b）查全率}[Performance comparison under different headways]{Performance comparison under different headways, which increases from 1 to 5 s. (a) Precision (b) Recall}
     \label{fig 6-6}
\end{figure}
