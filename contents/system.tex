...
通过公式 \ref{equ 5-10} 和公式\ref{equ 5-16}，并给定观测数据集$X^{(0)}$，可以得到稳定分布的四个估计参数。
...
图 \ref{fig 5-10} 比较了在不同数据包大小下 C-V2X 端到端时延，可以看到，虽然传输时延具有一定的波动性，但是最小的数据包（100 Bytes）和最大的数据包（7000 Bytes）的平均传输时延仍然为最小和最大的。
